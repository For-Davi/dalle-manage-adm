...
const isOpen = computed({
  get: () => props.open,
  set: (state: IFilterEnterprisePayment | 'close') => emit('update:open', state),
});
...
<template>
  <Dialog v-model:open="isOpen">
    <DialogContent  hide-close-button>
      <DialogHeader>
        <DialogTitle>
          <TitlePage title="Filtro de pagamentos das empresas" icon="LucideFunnel"/>
        </DialogTitle>
      </DialogHeader>
      ...
      <div class="mb-3 space-y-3">
        ...
        <Input v-model="dataEnterprisePayment.enterprise" id="enterprise" placeholder="Insira o nome da empresa" />
        </div>
        <div class="space-y-1">
              <Label>Status</Label>
      ...
      <div class="mt-3 flex justify-end">
        <Button type="button" @click="emit('update:open', 'close')" class="mr-2 cursor-pointer bg-red-300 hover:bg-red-400" :disabled="false">
          Fechar
        </Button>
        <Button type="submit" class="px-8 cursor-pointer" :disabled="false">
